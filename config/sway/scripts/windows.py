...
def build_dmenu_entries(windows):
    return enter.join(windows).encode("UTF-8")

# Executes menu with the given input string
def show_menu(windows):

    command="rofi -no-auto-select -i -dmenu -p \"Windows: \""

    process = subprocess.Popen(command,shell=True,stdin=subprocess.PIPE,stdout=subprocess.PIPE)
    return process.communicate(input=windows)[0]

# Returns the sway window id of the window that was selected by the user
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="window switcher")

    windows = get_windows()
    
    parsed_windows = parse_windows(windows)    
    
    dmenu_entries = build_dmenu_entries(parsed_windows)

    selected = show_menu(dmenu_entries)
